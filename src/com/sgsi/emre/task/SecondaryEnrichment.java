...
/**
 * Handles task Secondary Enrichment in EMRE LIMS v2.2 by
...
{
	String ID_COLUMN = "New LIMS ID*";
	String WORKSHEET = "Secondary Enrichment";
	/**
			...
			User user, Db db)
	{
		...
		if (ids.isEmpty())
		{
			...
			throw new LinxUserException(
					"No rows containing new enrichments were found in import file on worksheet tab '" + WORKSHEET + "'.");
		}

		// set up for standard core processing
		getServerItem(ItemType.SECONDARY_ENRICHMENT).setValues(ids);
		setMessage("Successfully logged " + ids.size() + " new enrichments. " 
	...
	public void doTaskWorkPostSave(HttpServletRequest request, HttpServletResponse response, User user, Db db)
	{
		...
		String sp = "spEMRE_bulkInsertSecondaryEnrichments";		

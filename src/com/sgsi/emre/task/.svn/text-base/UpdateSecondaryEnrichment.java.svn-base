...
/**
 * Handles task Update Secondary Enrichment in EMRE LIMS v2.2 by
...
public class UpdateSecondaryEnrichment extends EMRETask
{
	...
	String WORKSHEET = "Update Secondary Enrichment";
	/**
	...
	public void doTaskWorkPreSave(HttpServletRequest request, HttpServletResponse response, User user, Db db)
	{
		...
		String fileId = getServerItemValue(FileType.UPDATE_SECONDARY_ENRICHMENT_IMPORT_FILE);
		...
		setMessage("Successfully updated " + ids.size() + " enrichments. " 
				...
				+ " or click on task Secondary Enrichment in the workflow menu to autogenerate new IDs.");
	}
	...
	public void doTaskWorkPostSave(HttpServletRequest request, HttpServletResponse response, User user, Db db)
	{
		...
		String sp = "spEMRE_bulkUpdateSecondaryEnrichments";		

 ...
 */
  ...
  {
	String strain = getServerItemValue(ItemType.STRAIN);
	//if(bNewStrain) -- eff EMRE 2.2, allow editing any field except Strain ID
	//{
		setMessage("Successfully updated features for strain " + strain);
			...
			//lets save the gene additions
			 TableDataMap rowMap = new TableDataMap(request, ROWSET);
			  ...
			  for(int rowIdx = 1; rowIdx <= numRows; rowIdx++)
			  {
					  ...
					  {
						  throw new LinxUserException("Please enter values in all gene addition table columns (bottom of screen).");
					  }
		...
		if(!plasmidType.equalsIgnoreCase("none"))
		{
			...
			if(plasmidType.equalsIgnoreCase("exists"))
			{
				...
				dbHelper.updateStrainWithPlasmid(strainItemId, plasmidId, getTranId()+"",db);
			}
			else
			{
		...
		else
		{
			...
			if(!WtUtils.isNullOrBlankOrPlaceholder(plasmid) 
					...
					|| lsMarkers.size() != 0)
			{
				throw new LinxUserException("In order to add plasmid and plasmid marker data, please select a plasmid type.");
			}
		}
	//}
		/*

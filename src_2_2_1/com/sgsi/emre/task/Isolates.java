...
/**
 * Handles task Isolates in EMRE LIMS v2.2 by
 * parsing bulk import file and inserting new isolate properties via SQL bulk insert.
...
public class Isolates extends EMRETask
{
	...
	String WORKSHEET = "Isolates";
	/**
	 * Overridden to extract list of new isolate IDs from import file
	 * for std core processing.
	 * Eff EMRE LIMS v2.2 7/2011.
	 */
	@Override
	public void doTaskWorkPreSave(HttpServletRequest request, HttpServletResponse response,
			User user, Db db)
	{
		if (dbHelper == null)
		{
		...
		if (ids.isEmpty())
		{
			...
			throw new LinxUserException(
					"No rows containing new isolates were found in import file on worksheet tab '" + WORKSHEET + "'.");
		}

		// set up for standard core processing
		getServerItem(ItemType.ISOLATE).setValues(ids);
		setMessage("Successfully logged " + ids.size() + " new isolates. " 
	 ...
	 * and update custom tables with isolate properties. Relies
	 * on doTaskWorkPreSave() having found new isolate names and
	 * std core processing having created new isolates in ITEM.item.
	 * Eff EMRE LIMS v2.2 7/2011
	 */
	@Override
	public void doTaskWorkPostSave(HttpServletRequest request, HttpServletResponse response, User user, Db db)
	{
		...
		// extract list of isolates from user's file
		// core has already validated file per task def
		File inFile = this.getFile(fileId, db);
		...
		String sp = "spEMRE_bulkInsertIsolates";		

...
/**
 * Handles task Update Isolates in EMRE LIMS v2.2 by
 ...
 * 	-- screen input eliminated, ok'd per Jay
...
public class UpdateIsolates extends EMRETask
{
	...
	public void doTaskWorkPreSave(HttpServletRequest request, HttpServletResponse response, User user, Db db)
	{
		...
		if (ids.isEmpty())
		{
			...
			throw new LinxUserException("No rows containing existing isolates were found in import file on worksheet tab '" + WORKSHEET + "'.");
		}

		// set up for standard core processing
		getServerItem(ItemType.ISOLATE).setValues(ids);
		setMessage("Successfully updated " + ids.size() + " isolates. " 
				+ " Go to task 'Print Labels' to print labels,"
				+ " or click on task Isolates in the workflow menu to autogenerate new IDs.");
	}
	...
	public void doTaskWorkPostSave(HttpServletRequest request, HttpServletResponse response, User user, Db db)
	{
		...
		String sp = "spEMRE_bulkUpdateIsolates";		

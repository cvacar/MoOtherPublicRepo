 ...
 * @created 7/2008
 */
public class SvPlaceSampleonHold extends SvDefineProtocol
{
			...
			HttpServletRequest request, HttpServletResponse response)
	{
		if (request.getAttribute("ImportButton") != null)
		{
			try
			{
				...
				String fileId = task
						.getServerItemValue(FileType.HOLD_IMPORT_FILE);
			...
			HttpServletResponse response)
	{
		...
		if(xs.size() < 1)
			task.setMessage("There were no samples in the file therefore none were placed on hold.");
		else
		
			task.setMessage("Successfully imported samples to put on hold from bulk import file.");

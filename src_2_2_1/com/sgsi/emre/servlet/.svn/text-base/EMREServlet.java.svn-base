...
public class EMREServlet extends SGIServlet
{
  ...
  public RowsetView populateLocationsView(ArrayList<String> alLocations, RowsetView view, 
		  int colPrint, int colLoc, String tableName, Db db)
	{
	    ...
	    view = setupLocationsView(view, colPrint, colLoc, numRows);
  ...
  protected RowsetView setupLocationsView(RowsetView view, int colPrint, 
		  	...
		  	ArrayList<String> alLocations, Db db)
  {
	  ...
	  while(itor.hasNext())
	  {
		  ...
		  // write out a row in this UI table
		  String line = loc + Strings.CHAR.NEWLINE;
		  sb.append(line);
	  }
...
//			}
  ...
  /** 
   * When sending an In Clause to a stored procedure you need to delimit the single quote marks.
   * Convenience method: given a NodeList of value children,
   *  returns a SQL in(''..'',''..'',''..'') clause containing the list members,
   *  or "()" if no children.
   *  @param vList items to add to in clause
   */
  public static String getInClauseForStoredProc(List vList)
  {
    // may be ''
    String clause = "";
    int iValueCount = 0;
    ...
    {
      String sValue = (String)vList.get(i);
      if(WtDOMUtils.isNullOrBlank(sValue) )
      {
      	sValue = "";
      }
      else
      {
      	sValue = vList.get(i).toString();
      }
      if( iValueCount > 0 ){ clause += ","; }
      clause += "'" + sValue + "'";
      iValueCount++;
    }// next list member
    return clause;
  ...
  /**
   * Adds view to the list of views mapped 
   * to the session's 'rowsetViews' attribute, 
   * so that new views will be found consistently. 
   * Creates list if it doesn't exist, and adds
   * view if not already in the list.
   * @param request
   * @param view
   */
  public static void addViewToSessionViews(HttpServletRequest request, RowsetView view)
  {
    // list might not have been stored yet
    ArrayList list = RowsetView.getSessionViews(request);
    if( list == null)
    {
      list = new ArrayList();
      list.add(view);
      request.getSession().setAttribute(Strings.WIDGET.ROWSET_VIEWS, list);
      return;
    }
    
    // name-level compare, to replace old view obj
    for(int i = 0; i < list.size(); i++)
    {
      RowsetView v = (RowsetView)list.get(i);
      if (v.getWebsafeName().equals(view.getWebsafeName()))
      {
        // replace same-named view 
    ...
    // at exit, have found right view to replace, if present
    
    // new view
    list.add(view);
    // at exit, have found or created list of views, 
    // and added view if new
  }
  
	/**
	 * Downloads selected file to user as Excel-handled file.
	 * 
	 * @param fileId
	 * @param task
	 * @param request
	 ...
	 * @return
	 */
	protected boolean getFile(String fileId, Task task,
			HttpServletRequest request, HttpServletResponse response, User user, Db db)
	{
		try
		{
			// look for path value (may not have been provided)
			String path = dbHelper.getDbValue(
					"exec spLinx_getApplicationFileAndPath " + fileId, db);
			if (WtUtils.isNullOrBlank(path))
			{
				// a fairly common case
				throw new LinxUserException(
						"No path to the latest version has been stored for file " + fileId);
			}
			// file is known
			File file = new File(path);
			this.returnDownloadAsByteStream(response, file, file.getName(),
					AS_EXCEL_FILE, false);
			if (task.getTranId() == 0)
				task.getTranId(db);

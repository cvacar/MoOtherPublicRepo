...
public class SvReportAxenicStrainCollection extends EMREServlet 
{
	  ...
	  protected boolean handleCustomAction(Task task, User user, Db db, HttpServletRequest request, HttpServletResponse response)
	  {
	    ...
	    if(request.getParameter("Run")!= null)
	    {
	    	try
	    	{
	    		...
	    		String sql = "exec spMet_reportAxenicStrainCollection '";
	    		sql += task.getDisplayItemValue(ItemType.SECONDARY_ENRICHMENT) + "','";
	    		String well = task.getDisplayItemValue("WellNumber");
	    		if(!WtUtils.isNullOrBlank(well))
	    		{
	    			((EMRETask)task).validateWell(well);
	    			well = EMRETask.zeroPad(Integer.valueOf(well), 2);
	    		}

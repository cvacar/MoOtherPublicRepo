 ...
 * @created 7/2008
 */
public class SvSampleDepleted extends SvDefineProtocol
{
			...
			HttpServletResponse response, Task task, User user, Db db)
	{
		...
		task
				.setMessage("Edit Protocol values as needed, enter data, then click Save.");
			...
			HttpServletRequest request, HttpServletResponse response)
	{
		if (request.getAttribute("ImportButton") != null)
		{
			try
			{
				...
				String fileId = task
						.getServerItemValue(FileType.DEPLETION_IMPORT_FILE);
			...
			HttpServletResponse response)
	{
		...
		xs = imp.importXLSForBulkImport(inFile, task.getTaskName());

		// ready for std save() processing
		task
				.setMessage("Successfully imported depleted samples from bulk import file.");
		...
		while (itor.hasNext())
		{
			...
			task.getServerItem(ItemType.SAMPLE).setValue(
					x.getProperty("Depleted Sample ID"));

...
public class SvCultureSelection extends EMREServlet 
{
			...
			HttpServletRequest request, HttpServletResponse response)
	{
	  ...
	  protected void setNextId(Task task, Db db)
	  {
		...
		private boolean getFile(String fileId, Task task, HttpServletRequest request, 
			...
			throws Exception
		{
			try
			{
		        ...
		        String path = dbHelper.getDbValue("exec spLinx_getApplicationFileAndPath " + fileId, db);
		        if(WtUtils.isNullOrBlank(path))
		        {
		            // a fairly common case
		            throw new LinxUserException("No path to the latest version has been stored for imported file " + fileId);
		        }
		        // file is known
		        File file = new File(path);
		        this.returnDownloadAsByteStream(response, file, "CultureCollectionImport_" + fileId + ".xls", "application/vnd.ms-excel", false);
		        if(task.getTranId() == 0)
		        	task.getTranId(db);

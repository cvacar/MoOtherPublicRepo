...
public class SvStrainCollection extends EMREServlet// extends SvSampleReport
{
			...
			User user, Db db)
	{
		...
		{
			// in case of error,
			populateLocationsView(locationsView, COL_PRINT, COL_LOC, this
					.getLocationRowCount());
		}
		// else
		// {
	 ...
	 * table of existing data for a clone.
	 * 
	 * @param request
	 * @param response
	 * @param task
			...
			HttpServletResponse response, Task task, User user, Db db)
	{
		...
		if (request.getParameter("strain") != null)
		{
			...
			if (db.getHelper().isItemOnQueue(strain, ItemType.STRAIN,
					"Discarded Strains", db))
			{
				setDisplayItemValues(null, (StrainCollection) task, user, request, db);
				throw new LinxUserException("Strain " + strain
						+ " has been discarded and cannot be queried."
		...
		else
		{
			...
			if (fileId != null)
			{
				...
				this.returnDownloadAsByteStream(response, file, filename,
						AS_EXCEL_FILE, false);
				return ALL_DONE;
			}
			...
			HttpServletRequest request, HttpServletResponse response)
	{
		...
		try
		{
			...
			if (request.getAttribute("ImportButton") != null)
			{
				...
				setDisplayItemValues(null/* no strain */, (StrainCollection) task,
						user, request, db);
			...
			else if (request.getAttribute("NextIDButton") != null)
			{
				...
				// clear UI widgets
				setDisplayItemValues(null, (StrainCollection) task, user, request, db);
				String organism = task.getDisplayItemValue("Organism");
				if (WtUtils.isNullOrBlankOrPlaceholder(organism))
				{
					throw new LinxUserException(
							"Please select 'Strain type' from the list, then try again.");
				}
				String nextStrain = ((StrainCollection) task).getNextStrainID(organism, db);
				db = this.getDbObject(); // refresh conn

				task.getDisplayItem(ItemType.STRAIN).setValue(nextStrain);

				// now lets generate freezer positions for this new strain
				ArrayList<String> alLocations = ((StrainCollection) task)
						.reserveFreezerLocations(nextStrain, db);
				locationsView = populateLocationsView(alLocations, locationsView,
			...
			else if (request.getAttribute("ContentButton") != null)
			{
				try
				{
					...
					String url = dbHelper.getApplicationValue(db, "System Properties",
							"Strain Collection Get Content URL");
					// set the contenttype
					response.setContentType("text/html");
					// add the parameters for the get request
					url = url + "?external=True&username=" + user.getName()
							+ "&password=" + password + "&strain_id=" + strainId
							+ "&strain_name=" + strainName + "&project=" + project;
					// send this puppy off
	 ...
	 * @param response
	 */
	protected void handleCustomAction(Element domData, String strAction,
			Task task, User user, Db db, HttpServletRequest request,
			HttpServletResponse response)
	{
		...
		if (strAction.equalsIgnoreCase("verify strain exists"))
		{
			...
			{
				throw new LinxUserException(
						"Please provide a Strain name, then try again.");
			}
			if (db.getHelper().isItemExisting(strain, ItemType.STRAIN, db))
			{
		...
		super.handleCustomAction(domData, strAction, task, user, db, request,
				response);
	}
			...
			HttpServletRequest request, HttpServletResponse response)
	{
		try
		{
			...
			char delim = EMREStrings.CHAR.COMMA;// the data delimiter for the data
																					// container
			String columnKey = "Strain ID"; // the unique identifier that lets me know
																			// i've reach the column data in the file
			fileData = new XLSParser(inFile, "Strain", delim, columnKey,
					reqColumnHeaders, true);
			...
			try
			{
				if (fileData.gotoFirst())
				{
					do
					{
						...
						String notebook = fileData
								.getRequiredProperty("Page Ref [notebook-pg]");
						String loc1 = fileData
								.getRequiredProperty("Location 1 [Freezer:Box:Position]");
						String loc2 = fileData
								.getRequiredProperty("Location 2 [Freezer:Box:Position]");
						String loc3 = fileData
								.getRequiredProperty("Location 3 [Freezer:Box:Position]");
					...
					throw new Exception(
							"There were no rows found in the file.  Please check column headers and try again.");
			}
			catch (Exception e)
			{
				err += "Error occurred while parsing row " + row + ": "
						+ e.getMessage();
			}
			if (!err.equals(""))
	 ...
	 * data.
	 */
	protected void findStrainDataByStrain(HttpServletRequest request,
			DefaultTask task, User user, Db db)
	{
		String strain = task.getServerItemValue(ItemType.STRAIN);
		if (WtUtils.isNullOrBlank(strain))
		{
		...
		else if (!db.getHelper().isItemExisting(strain, ItemType.STRAIN, db))
		{
			...
			throw new LinxUserException("Strain " + strain
					+ " does not exist in this LIMS database."
					+ " Please check the entry, then try again.");
		}
		else if (db.getHelper().isItemOnQueue(strain, ItemType.STRAIN,
				"Discarded Strains", db))
		{
			setDisplayItemValues(null, task, user, request, db);
			throw new LinxUserException("Strain " + strain
	...
	/**
	 * Returns a rowset view of files previously imported for the selected strain.
	 * 
	 * @param request
	 * @param db
	 * @return rowset view of files for given strain
	 */
	protected RowsetView populateFilesView(String strain,
			HttpServletRequest request, Db db)
	{
		...
		RowsetView view = this.getSQLRowsetView(request, sql, "File Type",
			...
			User user, HttpServletRequest request, Db db)
	{
		...
		try
		{
			...
			if (dataFilesView.getRowcount() < 1)
			{
				...
				task
						.setMessage("No data files have been imported for strain " + strain);
			}
			else
			{
	 ...
	 * 'Print' and as many rows as required for this task's strain type (method is
	 * inherited by all strain collection tasks.) User can enter new locations, or
	 * click Find Strain to populate with known locations, or click Next ID to
	 * populate with next assumed available locations.
	 * 
	 */
	protected void initLocationsView(Db db)
	{
		...
		locationsView = this.getFileRowsetView(file, "Print", LOCATION_TABLE, null,
				colHeaders, numRows);
	...
	/**
	 * Basic setup of Location UI table, invoked by specific custom actions: Find
	 * Strain, Next ID, and new strain definition.
	 */
	 ...
	 * @return
	 */
	protected void populateLocationsView(String strain,
			HttpServletRequest request, Task task, Db db)
	{
		...
		{
			alLocations = ((StrainCollection) task).reserveFreezerLocations(strain,
					db);
			locationsView = this.populateLocationsView(alLocations, locationsView,
					COL_PRINT, COL_LOC, LOCATION_TABLE, db);
		}
		else
			locationsView = populateLocationsView(locationsView, COL_PRINT, COL_LOC,
					getLocationRowCount());
	...
	protected void findStrainLocations(String strain, HttpServletRequest request,
			Task task, Db db)
	{
		// show UI table of locations for selected strain
		RowsetView.cleanupSessionViews(request);
	...
	/**
	 * Write a file containing locations reserved for a strain, for showing in a
	 * Rowset table. See also method version that writes blank rows.
	 * 
	 * @param alLocations

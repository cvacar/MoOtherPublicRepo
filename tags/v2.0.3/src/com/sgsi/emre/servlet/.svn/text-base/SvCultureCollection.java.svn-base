...
public class SvCultureCollection extends EMREServlet 
{
	protected CultureCollection myTask = new CultureCollection();
	protected String cultureType = ItemType.STRAINCULTURE;
			...
			HttpServletResponse response)
	{
		...
		try
		{
			fileData = new XLSParser(inFile, task.getTaskName(),
					delim, columnKey, EMREStrings.GrowthRecovery.requiredStrainCultureCollectionColumnHeaders,
					true);
		}
		catch(Exception ex)
		{
		...
		try
		{
			if(fileData.gotoFirst())
			{
				do
				{			
					String limsId = fileData.getRequiredProperty("Strain Culture ID");
					String originLimsId = fileData.getRequiredProperty("Strain ID");

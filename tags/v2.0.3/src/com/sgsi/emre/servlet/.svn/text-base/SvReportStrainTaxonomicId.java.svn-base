...
public class SvReportStrainTaxonomicId extends EMREServlet 
{
			...
			HttpServletResponse response, Task task, User user, Db db)
	{
	     ...
	     {
	     	return handleExportRequest(request, response, DATA_TABLE, "StrainData_"+task.getTranId(db) +".csv");
	  ...
	  protected boolean handleCustomAction(Task task, User user, Db db, HttpServletRequest request, HttpServletResponse response)
	  {
	    ...
	    if(request.getParameter("Run")!= null)
	    {
	    	try
	    	{
	    		...
	    		String sql = "exec spMet_reportStrainTaxonomicId '";
	    		...
	    		if(WtUtils.isNullOrBlankOrPlaceholder(sampleType))
	    			sampleType = "";
	    		sql += sampleType + "','";
	    		String seqVendor = task.getDisplayItemValue("SequencingVendor");
	    		if(WtUtils.isNullOrBlankOrPlaceholder(seqVendor))
	    			seqVendor = "";
	    		sql += seqVendor + "','";
	    		sql += task.getDisplayItemValue("DateSubmitted") + "','";
	    		sql += task.getDisplayItemValue("VendorID") + "'";

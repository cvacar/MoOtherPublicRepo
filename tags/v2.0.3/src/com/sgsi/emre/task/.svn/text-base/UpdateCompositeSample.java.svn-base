...
public class UpdateCompositeSample extends UpdateEnvironmentalSample 
{
			  ...
			  User user, HttpServletRequest request, Db db)
		{
			...
			try
			{
				...
				ResultSet rs = db.getHelper().getResultSetFromStoredProc(db,
						"spEMRE_getCompositeSampleData", params, true);
				int numRows = 0;
				while (rs.next())
				{
					numRows++;
					...
					task.getDisplayItem("Depth_m").setValue(rs.getString(10));
					...
					task.getDisplayItem("StorageMethod").setValue(rs.getString(15));
					task.getDisplayItem("ClosestTown").setValue(rs.getString(16));
					task.getDisplayItem("City").setValue(rs.getString(17));
					task.getDisplayItem("County").setValue(rs.getString(18));
					task.getDisplayItem("State").setValue(rs.getString(19));
					task.getDisplayItem("Country").setValue(rs.getString(20));
				...
				if(numRows == 0)
					throw new LinxUserException("There were no rows returned from the database.\r\n" + 
							"Please ensure that you have a composite sample and try again.");
			}
			catch (SQLException e)
			{
				throw new LinxSystemException("While retrieving composite sample properties: "
						+ e.getMessage());
			}
		}
}

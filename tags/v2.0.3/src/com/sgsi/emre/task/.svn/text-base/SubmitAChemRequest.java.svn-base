		...
		try
		{
			if(fileData.gotoFirst())
			{
				do
				{
					...
					String cultureItemId = dbHelper.getItemId(culture, itemType, db);
					String cultureCollectionId = dbHelper.getDbValue("exec spEMRE_getCultureCollectionId " + cultureItemId, db);
					//does the timepoint exist?
					String samplingTimepointId = dbHelper.getDbValue("exec spEMRE_getSamplingTimepointId " + 
							cultureCollectionId + ",'" + timepoint + "'", db);
					//if not, add it
					if(WtUtils.isNullOrBlank(samplingTimepointId))
					{
						dbHelper.executeSQL("exec spEMRE_insertSamplingTimepoint " + 
								cultureCollectionId + ",'" + timepoint + "'," + this.getTranId(), db);
					}
...
//		if(itemType == null)
//		{
//			throw new LinxSystemException("The itemType was not set on the server.");
//		}
	...
	private void verifyItemMasks(String item, String type, String limsTracking, Db db)
	{
		try
		{
			...
			{
				String mask = "(WT|PH)-" + limssuffix + "[0-9]{4}-[0-9]{6}-[0-9]{3}(-Acid|-Base)?";
				Pattern p = Pattern.compile(mask);
				Matcher m = p.matcher(item);
				if(!m.matches())
			...
			{
				String mask = "EX-" + limssuffix + "[0-9]{4}-[0-9]{6}-[0-9]{3}(-Acid|-Base)?";
				Pattern p = Pattern.compile(mask);
				Matcher m = p.matcher(item);
				if(!m.matches())

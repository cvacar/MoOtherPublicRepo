...
public class SvLogEnvironmentalPlantSample extends SvLogEnvironmentalSample
{
	...
	public static final String[]	reqColHeaders		= new String[] { "New LIMS ID",
			"Physical Form", "Collection Date", "Notebook Ref", "Internal ID",
			"Field Name", "Description", "Volume (L)", "Weight (g)",
			"Temperature (degrees C)", "pH", "Dissolved Oxygen (mg/L)",
			"Conductivity (mS/m)", "Depth (m)", "Salinity (ppt)", "Latitude",
			"Longitude", "Altitude (m)", "Site Description", "Storage Method",
			...
			HttpServletRequest request, HttpServletResponse response)
	{
		...
		else if (request.getParameter("PrintLabel") != null)
		{
			...
			else if (!db.getHelper().isItemExisting(sample,
					ItemType.ENVIRONMENTAL_SAMPLE, db))
			{
				throw new LinxUserException("Sample " + sample
		...
		else if (request.getParameter("ExportButton") != null)
		{
			...
			writeToExcel(request, response, "exec spEMRE_exportEnvironmentalSample",
					db);

			return ALL_DONE;
		}
		return this.handleCustomAction(user, task, db, request, response);
	}


	/**
	 * Invoked by various tasks that used to all have a bulk import feature.
	 * In current release, some do and some don't, so can't assume that 
	 * request.getParameter() will return request params. 
	 * @param user
	 * @param task
	 * @param db
	 * @param request
	 ...
	 */
	protected boolean handleCustomAction(User user, Task task,  Db db,
			HttpServletRequest request, HttpServletResponse response)
	{
		...
		{
			// user wants to download the entire strain collection to Excel
			writeToExcel(request, response, "exec spMet_reportEnvironmentalSample",
					db);
			...
			HttpServletResponse response)
	{
		...
		catch (Exception ex)
		{
			...
			if(ex instanceof LinxDbException)
			{
	...
	// container
	// String columnKey = "New LIMS ID"; //the unique identifier that lets me know
	...
	protected void populateLocationsView(String sample,
			HttpServletRequest request, Task task, Db db)
	{
		...
		alLocations = reserveFreezerLocations(freezerLocSP, this.getBoxPrefix(db),
				db);
		ArrayList<String> alFrz = reserveFreezerLocations(freezerLocSP, this
				.getBackupBoxPrefix(db), db);
		for (String s : alFrz)
			alLocations.add(s);
		locationsView = super.populateFreezerLocationsView(locationsView,
				alLocations, rowCount, LOCATION_TABLE, COL_PRINT, COL_LOC, sql, sample,
				request, task, db);
		locationsView.setStartRow(1);
	...
	private void retrieveSubsurfaceSample(String sample, ResultSet rs, Task task,
			Db db)
	{
		try
		{
			while (rs.next())
			{
				...
				{
					throw new Exception("Sample " + sample
							+ " is not a subsurface sample.\r\n" + "Please choose the "
							+ rs.getString(2)
							+ " sample logging task to retrieve the sample information.");
				}
				else
				{
					task.getDisplayItem(ItemType.ENVIRONMENTAL_SAMPLE).setValue(
							rs.getString(1));
					...
					task.getDisplayItem("DissolvedOxygen_mg_per_L").setValue(
							rs.getString(17));
					task.getDisplayItem("Conductivity_mS_per_m").setValue(
							rs.getString(18));
	...
	private void retrieveCompositeSample(String sample, ResultSet rs, Task task,
			Db db)
	{
		try
		{
	...
	private void retrieveAquaticSample(String sample, ResultSet rs, Task task,
			Db db)
	{
		try
		{
	...
	private void retrieveAnimalSample(String sample, ResultSet rs, Task task,
			Db db)
	{
		try
		{

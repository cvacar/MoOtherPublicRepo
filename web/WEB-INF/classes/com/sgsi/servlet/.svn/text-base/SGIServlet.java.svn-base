...
{
	// file.separator \
	public String								fileSeparator		= System
	...
	/**
	 * Returns the 'action' attribute's value from a servlet request. Use other
	 * method signature if XML request action is wanted.
	 * 
	 * @param request
	...
	/**
	 * Linx 2 requires certain elements under any XML client request, but ignores
	 * additional custom nodes. Robot uses a custom 'data' node to pass custom
	 * data between client and LIMS server. This convenience method returns the
	 * value of the first child found under 'data' with dataChildName. So, if the
	 * tree looks like:
	 * 
	 * <pre>
	...
	protected String getDataChildValue(Element domData, String dataChildName)
	{
		...
		if (eData == null)
		{
			throw new LinxUserException("No '" + dataChildName
					+ "' child element found under given DOM. "
	...
	{
		Document doc = WtDOMUtils
				.getDocumentWithoutProcessingInstructions(WtDOMUtils.getBuilder());
	 ...
	 *          task == null ) { throw new
	 *          LinxSystemException("Cannot execute save() on null task." +
	 *          Strings.MSG.ALERT_LIMS_ADMIN); }
	 * 
	 *          Code.event("Running task " + task.getTaskName() +
	 *          " with tranId = " + task.getTranId());
	 * 
	 *          // catch single-item verifies if( task.getServerItems().size() > 1
	 *          && task.isSingleItemVerify() ) { task.verifyItems(request,
	 *          response, user, db); // single-item verify successful (throws on
	 *          error) return; }
	 * 
	 *          // default message can be overwritten by any method
	 *          task.recordTaskHistory(request, response, getWorkflow(), user,
	 *          db); request.getSession().setAttribute("lastTranId",
	 *          String.valueOf(task.getTranId()));
	 * 
	...
	public void writeToExcel(HttpServletRequest request,
			HttpServletResponse response, String sql, Db db)
	{
		...
		catch (Exception ex)
		{
			throw new LinxUserException(
					"Error occurred while attempting to write to MS Excel:  " + ex);
		}
	 ...
	 * @param sql
	 */
	public void writeToExcelWithInlineHeaders(HttpServletRequest request,
			HttpServletResponse response, String sql, Hashtable headerValues, Db db)
	{
		if (WtUtils.isNullOrBlank(sql))
		{
			throw new LinxUserException("Unable to write to MS Excel.\r\n"
	...
	/**
	 * Returns the input number reformatted for the given number of decimal
	 * places.
	 * 
	 * @param sNum
	...
	public String formatNumber(String sNum, int numDecimalPlaces)
	{
		...
		{
			throw new LinxUserException("Value " + sNum + " could not be formatted: "
					+ ex.getMessage());
		}
		return sOut;
	}
	...
	/**
	 * Excutes a sql statement and writes out the results to a uniquely named file
	 * using RChart delimiter, returning the data file name.
	 * 
	 * @param request
	 * @param response
	 * @param sql
	...
	public String writeToChartSourceFile(String sql, Db db)
	{
		...
		{
			throw new LinxUserException(
					"Error occurred while attempting to write to excel.  " + ex);
		}
		return chartFile.getName();
	}

	/**
	 * Sends the contents of the given file back to the client as a byte stream,
	 * usually prompting a 'Save As...' dialog from the browser. See also version
	 * that takes a StringBuffer.
	 * 
	 * @param response
	 * @param file
	 *          any character-based file
	 * @param strTargetName
	 *          suggested file name to show user
	 * @param strContentType
	 *          usually 'text/txt' or other content-type recognizable to client
	 * @param bDeleteFileOnExit
	 *          true if the given file can be safely deleted after transfer
	 */
	protected void returnDownloadAsByteStream(HttpServletResponse response,
			File file, String strTargetName, String strContentType,
			boolean bDeleteFileOnExit)
	{
	...
	/**
	 * Defines the file type, and its current location, input path, and output
	 * path.
	 */
	...
	/**
	 * Returns the file stored for this appFileId, provided LIMS server can access
	...
	{
		String filepath = dbHelper.getDbValue(
				"exec spLinx_getApplicationFileAndPath " + appFileId, db);
		return new File(filepath);
	}

	/**
	 * Gets the result set as a CSV file as a view which the rowsetView widget can
	 * deal with.
	 * 
	 * @param request
	 *          The current request
	 ...
	 *          The current task
	 * @param sql
	 *          The sql to populate the rowset with
	 * @param sRefHeader
	 *          The first header expected in the file
	 * @param sTableName
	 *          The name of the table -- may be null
	 * @param maxRows
	 *          acts like 'top n' in SQL
	 ...
	 */
	public DefaultTableView getSQLRowsetView(HttpServletRequest request,
			String procName, ArrayList<String> params, String sRefHeader,
			String sTableName, int maxRows, Db db)
	{
	...
	/**
	 * Uses a CSV file to create a rowsetView table for display from a JSP.
	 * 
	 * @param request
	 *          The current request
	 * @param file
	 *          The file containing data to build the table from
	 * @param headerNames
	 *          A list of headers names in the file
	 * @param db
	 *          The
	 * @return
	 */
	...
	/**
	 * A dummy source file for drawing numSectors number of rows on the UI, with
	 * colHeaders in format "Header1,Header2,Header3,...".
	 * 
	 * @param numRows
	 * @param colHeaders
